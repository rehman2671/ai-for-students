...
function LessonPicker({ onStart, best }: { onStart: (lesson: GameLesson) => void; best: number }) {
  return <div className="game-picker"><div className="game-picker-heading"><span className="game-kicker">PROMPT DETECTIVE / FIELD GAME 01</span><h2>Train your eye<br /><em>for better prompts.</em></h2><p>Fifteen cases. Three choices. A clearer way to learn what makes an AI request useful.</p></div><div className="lesson-grid">{lessons.map((item, index) => <button className="lesson-card" key={item.id} onClick={() => onStart(item.id)}><span className="lesson-number">0{index + 1}</span><strong>{item.label}</strong><small>{item.description}</small><span className="lesson-start">Open case <ArrowRight size={14} /></span></button>)}</div>{best > 0 && <div className="best-score"><Sparkles size={15} /> Best desk score: <strong>{best}/15</strong></div>}</div>;
}
