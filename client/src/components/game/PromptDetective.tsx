...
export default function PromptDetective() {
  ...
  if (index >= round.length) {
    ...
    return <div className="game-result"><div className="result-mark"><Sparkles size={26} /></div><span className="game-kicker">CASE CLOSED</span><h2>{finalScore}<small> / {round.length}</small></h2><p className="result-message">{message}</p><div className="result-note"><strong>Your next move</strong><span>Pick one prompt from the shelf and adapt it to something you are learning today.</span></div><div className="game-result-actions"><button className="game-button game-button--primary" onClick={() => start(lesson)}><RotateCcw size={16} /> Play again</button><button className="game-button game-button--quiet" onClick={reset}>Choose another lesson</button></div></div>;
  }
