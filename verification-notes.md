...
Mobile visual QA after stronger shuffle: AI Safety Lab, Fact Check Quest, Prompt Detective and Source Hunt all render at 390px width. Search, difficulty, age-band, topic and skill controls stack cleanly; game cards remain readable and the route header/back affordance stays visible. No answer-order or layout regression was visible in the captured routes.

Connected-browser self-check: the Safety Lab guest route loaded with all discovery controls and twelve game modules visible; the account session was already signed in. A direct scroll and demo-route navigation then hit the browser extension 504 timeout, so no sensitive sign-in or account mutation was attempted. Source-level and automated replay coverage remains the reliable verification for the answer-order change.
